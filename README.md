...
bundle exec ./ios/scripts/generate-project.sh   # xcodegen generate + pod install
```

(Run `bundle install` once first to install CocoaPods via the Gemfile.)

The native engine is built as a locally generated static XCFramework at:

```text
ios/ThirdParty/Libbox.xcframework
```

The Xcode `LibboxKit` target force-loads that archive into a dynamic framework.
The host app embeds one copy of `LibboxKit.framework`, and both the app and
PacketTunnel extension link it. The XCFramework is Git-ignored.

Build one unified device+simulator framework from the pinned
